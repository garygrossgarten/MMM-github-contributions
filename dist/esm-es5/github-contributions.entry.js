...
var lib$1 = function parseGitHubCalendarSvg(input) {
    ...
    }, lastWeek = [], updateLongestStreak = function updateLongestStreak() {
    ...
    input.split("\n").slice(2).map(function (c) {
        ...
        var fill = c.match(/fill="(#[a-z0-9]+)"/), date = c.match(/data-date="([0-9\-]+)"/), count = c.match(/data-count="([0-9]+)"/);
        fill = fill && fill[1];
        date = date && date[1];
        count = count && +count[1];
        if (!fill) {
...
var DATE_FORMAT1 = "MMM D, YYYY", DATE_FORMAT2 = "MMMM D";
    ...
    var fetchCalendar = function fetchCalendar() {
        return fetch(options.proxy("https://github.com/users/" + username + "/contributions")).then(function (response) {
            ...
            if (cal.querySelector("include-fragment")) {
                ...
                if (options.global_stats !== false) {
                    var parsed = lib$1(lib$3("svg", cal).outerHTML), currentStreakInfo = parsed.current_streak ? lib$8(parsed.current_streak_range[0], DATE_FORMAT2) + " &ndash; " + lib$8(parsed.current_streak_range[1], DATE_FORMAT2) : parsed.last_contributed ? "Last contributed in " + lib$8(parsed.last_contributed, DATE_FORMAT2) + "." : "Rock - Hard Place", longestStreakInfo = parsed.longest_streak ? lib$8(parsed.longest_streak_range[0], DATE_FORMAT2) + " &ndash; " + lib$8(parsed.longest_streak_range[1], DATE_FORMAT2) : parsed.last_contributed ? "Last contributed in " + lib$8(parsed.last_contributed, DATE_FORMAT2) + "." : "Rock - Hard Place", firstCol = lib$3("<div>", {
